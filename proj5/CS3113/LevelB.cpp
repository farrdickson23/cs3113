...
Vector2 BspawnPos1 = { 0.f, 0.f };
Vector2 BspawnPos2 = { 0.f, 0.f };
Vector2 BspawnPos3 = { 0.f, 0.f };
Vector2 BspawnPos4 = { 0.f, 0.f };
Vector2 BspawnPos5 = { 0.f, 0.f };
//bool trap = true;
...
//bool enemySpawned[5] = { false, false, false, false, false };
...
void LevelB::initialise()
{
    ...
    //for (int i = 0; i < 5; ++i) enemySpawned[i] = false;
    ...
    */
    ...
    BspawnPos1 = { mGameState.map->getLeftBoundary() + 225 ,
                   mGameState.map->getBottomBoundary() - 225};
    BspawnPos3 = { mGameState.map->getRightBoundary() - 225 ,
                   mGameState.map->getBottomBoundary() - 225 };
    BspawnPos4 = { mGameState.map->getLeftBoundary() + 225 ,
                   mGameState.map->getTopBoundary() + 225 };

    //BspawnPos4 = { -(mGameState.map->getLeftBoundary()) + 200, mGameState.map->getTopBoundary() + (mGameState.map->getBottomBoundary() - mGameState.map->getTopBoundary()) / 2 };
    ...
    */
    ...
    BspawnPos2 = { mGameState.map->getBottomBoundary()/2, mGameState.map->getRightBoundary() / 2 };
   ...
   // mGameState.chest1->isOpen();
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i == 0) {
            ...
            mGameState.enemies[i]->setAIState(IDLE);
            mGameState.enemies[i]->setSpeed(40);
            mGameState.enemies[i]->setColliderDimensions({
                mGameState.xochitl->getScale().x / 3.5f,
                mGameState.xochitl->getScale().y / 3.0f
                });
...
void LevelB::update(float deltaTime)
{
    ...
    if (sniper < 0.f) {
        sniper = 10.f;
        if (survive > 20.f ) {
            mGameState.enemies[0]->setPosition( BspawnPos1);
        } else if (survive > 10.f ) {
            mGameState.enemies[0]->setPosition(BspawnPos3);
        } else if (survive > 0.f) {
            mGameState.enemies[0]->setPosition(BspawnPos4);
        }
        mGameState.enemies[0]->activate();

    }
    respawnTimer -= deltaTime;
    if (survive > 0.f && respawnTimer < 0.f) {
        respawnTimer = 5.f;
        if (!mGameState.enemies[1]->isActive()) {
            //kills += 1;
            mGameState.enemies[1]->activate();
            mGameState.enemies[1]->setSpeed(50);
            mGameState.enemies[1]->setPosition({ mGameState.map->getLeftBoundary() +225, mGameState.map->getBottomBoundary()/2 });
        }
        else if (!mGameState.enemies[2]->isActive()) {
            //kills += 1;
            mGameState.enemies[2]->activate();
            mGameState.enemies[2]->setSpeed(50);
            mGameState.enemies[2]->setPosition({ mGameState.map->getRightBoundary() /2 , mGameState.map->getTopBoundary() +225 });
        }
        else if (!mGameState.enemies[3]->isActive()) {
            //kills += 1;
            mGameState.enemies[3]->activate();
            mGameState.enemies[3]->setSpeed(50);
            mGameState.enemies[3]->setPosition({ mGameState.map->getRightBoundary()/2, mGameState.map->getBottomBoundary() -225 });
        }
        else if (!mGameState.enemies[4]->isActive()) {
            //kills += 1;
            mGameState.enemies[4]->activate();
            mGameState.enemies[4]->setSpeed(50);
            mGameState.enemies[4]->setPosition({ mGameState.map->getRightBoundary() - 225, mGameState.map->getBottomBoundary() / 2 });
        }
    }
...
void LevelB::render()
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (mGameState.enemies[i]->isActive() && mGameState.enemies[i]->isAiming()) {
            mGameState.enemies[i]->aimingTimer += BtimeChange;
