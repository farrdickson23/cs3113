...
void LevelC::initialise()
{
    ...
    */
    ...
    mGameState.monster1 = new Entity(
        ...
        { 150.0f * sizeRatio, 150.0f },             // scale
        "assets/game/slime.png",                   // texture file address
        ATLAS,                                    // single image or atlas?
    ...
    mGameState.monster1->setColliderDimensions({
       mGameState.monster1->getScale().x / 1.f,
       mGameState.monster1->getScale().y / 1.f
        });
    mGameState.monster1->setAcceleration({ 0.0f, ACCELERATION_OF_GRAVITY/3 });
    mGameState.monster1->setJumpingPower(500.f);

    mGameState.monster1->setAIType(BOUNCER);
    mGameState.monster1->setAIState(SIDLE);

    mGameState.monster1->setSpeed(Entity::DEFAULT_SPEED +5 ); // the monster is going to fast
...
void LevelC::update(float deltaTime)
{
    ...
    if ((mGameState.ninja->isColliding(mGameState.monster1)) ) // i dont remember why we had this private but hopefully this isn't too big of an issue oh whale
    {
    ...
    if (mGameState.ninja->getPosition().y > 800.0f) mGameState.nextSceneID = 4; // this should go to boss scene
